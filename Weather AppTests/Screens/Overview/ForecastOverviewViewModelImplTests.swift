...
class ForecastOverviewViewModelImplTests: XCTestCase {
    ...
    func testLoadDailyForecast() throws {
        ...
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .dailyForecast), generatedData.map { .daily($0, 21.14, 23.64, generatedData) })
    }
    ...
    func testLoadedBoth() throws {
        ...
        XCTAssertEqual(snapshot.itemIdentifiers(inSection: .dailyForecast), generatedDailyForecasts.map { .daily($0, 21.14, 23.64, generatedDailyForecasts) })
    }
}
